...
Imported['VE - Diagonal Movement'] = '1.01';
...
/*:
 *------------------------------------------------------------------------------ 
 * @plugindesc v1.01 - Allows diagonal movement for player and events.
 * @author Victor Sant
...
(function() {
	...
	if (Imported['VE - Basic Module']) {
		...
		VictorEngine.Parameters.DiagonalAdjust    = eval(parameters["Diagonal Adjust"]);
		VictorEngine.Parameters.DiagonalEvents    = eval(parameters["Diagonal Events"]);
		VictorEngine.Parameters.DiagonalFix       = eval(parameters["Diagonal Fix"]);
		VictorEngine.Parameters.DiagonalTrigger   = eval(parameters["Diagonal Trigger"]);
		VictorEngine.Parameters.CornerPassability = eval(parameters["Corner Passability"]);
	};
	...
	Game_CharacterBase.prototype.distancePerFrame = function() {
		...
		var adjust   = VictorEngine.Parameters.DiagonalAdjust;
	...
	Game_CharacterBase.prototype.canPassDiagonally = function(x, y, horz, vert) {
		if (VictorEngine.Parameters.CornerPassability) {
	...
	Game_CharacterBase.prototype.checkEventTriggerTouchFront = function(d) {
		if (VictorEngine.Parameters.DiagonalTrigger && d % 2 !== 0){
			var horz = ((d === 1 || d === 7) ? 4 : 6);
			var vert = ((d === 1 || d === 3) ? 2 : 8);
	...
	Game_Character.prototype.needDiagonalFix = function() {
		return (!this.isMoving() && VictorEngine.Parameters.DiagonalFix);
	};
	...
	Game_Player.prototype.checkEventTriggerThere = function(triggers) {
		if (VictorEngine.Parameters.DiagonalTrigger && this.isDiagonal()){
	...
	Game_Event.prototype.enableDiagonalEvent = function() {
		...
		if (VictorEngine.Parameters.DiagonalEvents || regex.test(note)) {
			this.setDiagonalOn();
		};
	};	
